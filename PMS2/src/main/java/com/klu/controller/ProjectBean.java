...
public String ApprovedForm()
{
	Project p=new Project();
	p.setApprovedby(approvedby);
	p.setEmail(Email);
	p.setProjectID(projectID);
	response=pb.approveProject(p);
	return "Darshan";
}
public List<Project> displayAllProjects()
{
	list=pb.getAllProjectDetails();
	return list;
}
public List<Project> displayApprovedProjects()
{
	list=pb.getAllApprovedProjectDetails(Email);
	return list;
}
}
